...
function startTimer(lessonValue, restValue, countValue) {
  ...
  function lessonTimer() {
    ...
    let lessonInterval = setInterval(() => {
      ...
      taymer.innerHTML = `
        <span>Dars qilish vaqti: ${minutes}:${seconds < 10 ? '0' + seconds : seconds}</span>
      `;
  ...
  function restTimer() {
    ...
    let restInterval = setInterval(() => {
      ...
      taymer.innerHTML = `
        <span>Dam olish vaqti: ${minutes}:${seconds < 10 ? '0' + seconds : seconds}</span>
      `;
      if (restTime <= 0) {
        ...
        if (cycle < countValue) {
          ...
          taymer.innerHTML = "Jarayon tugadi!";
        }
      } else {
        restTime--;
      }
